"""
Script to fetch UK sovereign yield curve data from worldgovernmentbonds.com
...
Data source: https://www.worldgovernmentbonds.com/country/united-kingdom/

Run: python scripts/fetch_uk_yield_curve.py
"""
...
def parse_percentage(text):
    """Parse percentage string like '4.129%' to float"""
    if not text:
        return None
...
def parse_basis_points(text):
    """Parse basis points like '+3.5 bp' or '-9.8 bp' to float"""
    if not text:
        return None
...
def parse_maturity_to_years(maturity_text):
    """Parse maturity text like '1 month', '3 months', '1 year', '2 years' to years as float"""
    if not maturity_text:
        return None

    maturity_text = maturity_text.strip().lower()

    # Match patterns like "1 month", "3 months", "1 year", "2 years"
...
def fetch_yield_curve_playwright():
    """
    Fetch UK yield curve data using Playwright (headless browser)
    ...
    url = "https://www.worldgovernmentbonds.com/country/united-kingdom/"
    ...
    try:
        with sync_playwright() as p:
            ...
            print(">> Loading page...")
            page.goto(url, wait_until="domcontentloaded")  # Use domcontentloaded instead of networkidle

            # Wait for the yield curve data to load
            print(">> Waiting for data to load...")
            time.sleep(15)  # Wait longer for JavaScript to fully render
            ...
            debug_file = Path('uk_page_debug.html')
            ...
            if yields_data:
                ...
                print(">> Check uk_page_debug.html to see page structure")
                return None
...
def save_yield_curve(data, config):
    ...
    output_file = markets_dir / 'UK_Yield_Curve.json'
...
def save_historical_snapshot(data, config):
    ...
    json_dir = Path(config.JSON_DIR)
    history_dir = json_dir / 'Markets' / 'UK_Yield_Curve_History'
    history_dir.mkdir(exist_ok=True)

    # Create filename with date
    today = datetime.now().strftime('%Y-%m-%d')
    history_file = history_dir / f'UK_Yield_Curve_{today}.json'
...
def main():
    ...
    print("=" * 70)
    print("UK Sovereign Yield Curve Data Fetcher")
    ...
    if not scraped_data:
        ...
        print(">> Check uk_page_debug.html to inspect page structure")
