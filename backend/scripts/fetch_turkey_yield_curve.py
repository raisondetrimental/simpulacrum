"""
Script to fetch Turkey sovereign yield curve data from worldgovernmentbonds.com
...
Run: python scripts/fetch_turkey_yield_curve.py
"""
...
def parse_percentage(text):
    """Parse percentage string like '35.107%' to float"""
    if not text:
        return None
...
def parse_basis_points(text):
    """Parse basis points like '-316.4 bp' or '+283.0 bp' to float"""
    if not text:
        return None
...
def parse_date(text):
    """Parse date string like '14 Nov' to standardized format"""
    if not text:
        return None
    text = text.strip()
...
def parse_maturity_to_years(maturity_text):
    """Parse maturity text like '3 months', '2 years' to years as float"""
    if not maturity_text:
        return None

    maturity_text = maturity_text.strip().lower()

    # Match patterns like "3 months" or "2 years"
...
def extract_yield_curve_table(soup):
    ...
    for table_idx, table in enumerate(tables):
        ...
        for row_idx, row in enumerate(rows):
            ...
            if 'residual' in row_text or 'annualized' in row_text or 'constant' in row_text:
...
def fetch_yield_curve_playwright():
    """
    Fetch Turkey yield curve data using Playwright (headless browser)
    ...
    try:
        with sync_playwright() as p:
            ...
            debug_file = Path('turkey_page_debug.html')
            ...
            if yields_data:
                ...
                print(">> Check turkey_page_debug.html to see page structure")
                return None
...
def save_yield_curve(data, config):
    ...
    output_file = markets_dir / 'Turkey_Yield_Curve.json'
...
def save_historical_snapshot(data, config):
    ...
    json_dir = Path(config.JSON_DIR)
    history_dir = json_dir / 'Markets' / 'Turkey_Yield_Curve_History'
    history_dir.mkdir(exist_ok=True)

    # Create filename with date
    today = datetime.now().strftime('%Y-%m-%d')
    history_file = history_dir / f'Turkey_Yield_Curve_{today}.json'
...
def main():
    ...
    print("=" * 70)
    print("Turkey Sovereign Yield Curve Data Fetcher")
    ...
    if not scraped_data:
        ...
        print(">> Check turkey_page_debug.html to inspect page structure")
