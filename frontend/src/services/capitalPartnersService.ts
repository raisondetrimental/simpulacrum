...
export function exportCapitalPartnersToCSV(partners: CapitalPartner[]): string {
  ...
  const rows = partners.map(partner => [
    ...
    partner.preferences?.vietnam || '',
    partner.preferences?.mongolia || '',
    partner.preferences?.turkey || '',
...
export async function downloadCapitalPartnersXLSX(): Promise<void> {
  ...
  const data = response.data.map(partner => ({
    ...
    'Vietnam': partner.preferences?.vietnam || '',
    'Mongolia': partner.preferences?.mongolia || '',
    'Turkey': partner.preferences?.turkey || '',
