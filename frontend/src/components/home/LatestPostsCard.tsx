...
const LatestPostsCard: React.FC = () => {
  ...
  const loadPosts = async () => {
    try {
      const { posts: fetchedPosts, error } = await getTopGeneralPosts(6);
      if (!error) {
            ...
            <div className="space-y-2 flex-1 overflow-y-auto">
              {posts.map((post) => (
                <Link
                  ...
                  className="block p-3 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border border-green-200 hover:border-green-400 hover:shadow-sm transition-all"
                >
                  ...
                  <p className="text-xs text-gray-700 mb-2">
                    {truncateText(stripHtml(post.content), 90)}
                  </p>

                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-600">{post.full_name}</span>
