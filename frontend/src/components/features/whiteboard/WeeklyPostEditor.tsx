/**
 * Weekly Post Editor Modal
 * Modal for creating/editing weekly whiteboard posts
...
import type { WeeklyPost, WeeklyPostFormData } from '../../../types/whiteboard';
import type { User } from '../../../types/users';

interface WeeklyPostEditorProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: WeeklyPostFormData & { user_id?: string; full_name?: string }) => Promise<void>;
  post?: WeeklyPost | null;
  weekLabel: string;
  preSelectedUserName?: string; // For admin creating post for specific user
}

const WeeklyPostEditor: React.FC<WeeklyPostEditorProps> = ({
  isOpen,
  onClose,
  onSave,
  post,
  ...
  // Initialize form with existing post data when editing or pre-selected user
  useEffect(() => {
    if (post) {
      setTitle(post.title);
  ...
  // Pre-select user when creating for specific user (admin feature)
  useEffect(() => {
    if (preSelectedUserName && users.length > 0 && !post) {
      const matchingUser = users.find(u => u.full_name === preSelectedUserName);
      if (matchingUser) {
        setSelectedUserId(matchingUser.id);
        setSelectedUserFullName(matchingUser.full_name);
      }
    }
  }, [preSelectedUserName, users, post]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const postData: WeeklyPostFormData & { user_id?: string; full_name?: string } = {
        title,
        content,
      };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 rounded-t-lg">
          <h2 className="text-2xl font-bold text-gray-900">
            {post ? 'Edit Weekly Post' : preSelectedUserName ? `Create Post for ${preSelectedUserName}` : 'New Weekly Post'}
          </h2>
...
export default WeeklyPostEditor;
