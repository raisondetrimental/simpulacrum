/**
 * Counsel Reminders Widget
 * Shows upcoming counsel contact follow-ups on counsel landing page
 */

import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { CounselContact, ApiResponse } from '../../../types/counsel';
...
const CounselReminders: React.FC = () => {
  ...
  const fetchReminders = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/counsel-meetings/reminders`);
    ...
    <div className="space-y-3">
      {reminders.map((reminder) => {
        ...
        return (
          <Link
            ...
            to={`/counsel/contacts/${reminder.contact.id}`}
      ...
      {reminders.length > 0 && (
        <Link
          ...
          className="block text-center text-sm text-purple-600 hover:text-purple-800 pt-2"
        >
          View all legal advisors →
        </Link>
      )}
    </div>
  );
...
export default CounselReminders;
