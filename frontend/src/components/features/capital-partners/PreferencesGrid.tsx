 ...
 * Reusable component for displaying/editing investment preferences
 * Used in Teams detail page and Meeting Notes page
 */
...
interface PreferencesGridProps {
  ...
  onChange?: (preferences: Partial<InvestmentPreferences>) => void;
  readonly?: boolean;
  collapsible?: boolean;
}
...
const PreferencesGrid: React.FC<PreferencesGridProps> = ({
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(
    new Set(PREFERENCE_GROUPS.map(g => g.title))
  );
  ...
  const handleToggle = (key: keyof InvestmentPreferences) => {
  ...
  const expandAll = () => {
    setExpandedGroups(new Set(PREFERENCE_GROUPS.map(g => g.title)));
  };
