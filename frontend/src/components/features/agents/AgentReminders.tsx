/**
 * Agent Reminders Widget
 * Shows upcoming agent contact follow-ups on agents landing page
 */

import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { AgentContact, ApiResponse } from '../../../types/agents';
...
const AgentReminders: React.FC = () => {
  ...
  const fetchReminders = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/agent-meetings/reminders`);
  ...
  return (
    <div className="space-y-3">
      {reminders.map((reminder) => {
        ...
        return (
          <Link
            ...
            to={`/agents/contacts/${reminder.contact.id}`}
      ...
      {reminders.length > 0 && (
        <Link
          to="/agents/list"
          className="block text-center text-sm text-orange-600 hover:text-orange-800 pt-2"
        >
          View all agents →
        </Link>
      )}
