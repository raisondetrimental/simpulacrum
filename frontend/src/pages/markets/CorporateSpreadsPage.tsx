...
import { getCorporateSpreads, refreshCorporateSpreads, CorporateSpreadsData } from '../../services/marketsService';

const CorporateSpreadsPage: React.FC = () => {
  const [data, setData] = useState<CorporateSpreadsData | null>(null);
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const spreadsData = await getCorporateSpreads();
      ...
      const filteredData = {
        ...spreadsData,
        data: spreadsData.data.filter((row: any) => {
  ...
  const handleRefresh = async () => {
    try {
      ...
      const result = await refreshCorporateSpreads();
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-gray-600">Loading corporate spreads data...</div>
      </div>
    );
  }
  ...
  const globalHYChange = latestData.global_hy && previousData?.global_hy
    ...
    : null;
  const globalIGChange = latestData.global_ig && previousData?.global_ig
    ? latestData.global_ig - previousData.global_ig
    : null;
  ...
  const CATEGORY_LABELS: { [key: string]: string } = {
    ...
    global_ig: 'Global Investment Grade',
  ...
  const calculateChanges = (category: CategoryKey) => {
    ...
    const dailyChange = current && oneDayAgo?.[category]
      ? (current - oneDayAgo[category]!) * 1  // Already in bps
      : 0;
    ...
    const weeklyChange = current && oneWeekAgo?.[category]
      ? (current - oneWeekAgo[category]!) * 1  // Already in bps
      : 0;
    ...
    const monthlyChange = current && oneMonthAgo?.[category]
      ? (current - oneMonthAgo[category]!) * 1  // Already in bps
      : 0;
  ...
  return (
    <div className="space-y-8 p-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Corporate Bond Spreads (OAS)</h1>
          <p className="text-sm text-gray-600 mt-1">
            ICE BofA Option-Adjusted Spreads (basis points) - Last 90 Days
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {latestData.global_hy?.toFixed(0) ?? 'N/A'} <span className="text-lg">bps</span>
          </p>
          {globalHYChange !== null && (
            <p className={`text-sm mt-2 ${globalHYChange >= 0 ? 'text-red-600' : 'text-green-600'}`}>
              {globalHYChange >= 0 ? '+' : ''}{globalHYChange.toFixed(0)} bps
            </p>
          )}
        </div>
        ...
        {/* Global IG */}
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-sm font-semibold text-gray-600 mb-2">Global Investment Grade</h3>
          <p className="text-3xl font-bold text-gray-900">
            {latestData.global_ig?.toFixed(0) ?? 'N/A'} <span className="text-lg">bps</span>
          </p>
          {globalIGChange !== null && (
            <p className={`text-sm mt-2 ${globalIGChange >= 0 ? 'text-red-600' : 'text-green-600'}`}>
              {globalIGChange >= 0 ? '+' : ''}{globalIGChange.toFixed(0)} bps
            </p>
          )}
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <p className="text-3xl font-bold text-gray-900">
            {latestData.em_corporate?.toFixed(0) ?? 'N/A'} <span className="text-lg">bps</span>
          </p>
          {emCorporateChange !== null && (
            <p className={`text-sm mt-2 ${emCorporateChange >= 0 ? 'text-red-600' : 'text-green-600'}`}>
              {emCorporateChange >= 0 ? '+' : ''}{emCorporateChange.toFixed(0)} bps
            </p>
          )}
        </div>
        ...
        {/* HY-IG Spread */}
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h3 className="text-sm font-semibold text-gray-600 mb-2">HY-IG Spread</h3>
          <p className="text-3xl font-bold text-gray-900">
            {latestData.global_hy && latestData.global_ig
              ? (latestData.global_hy - latestData.global_ig).toFixed(0)
              : 'N/A'} <span className="text-lg">bps</span>
          </p>
          {latestData.global_hy && latestData.global_ig && previousData?.global_hy && previousData?.global_ig && (
            <p className={`text-sm mt-2 ${
              (latestData.global_hy - latestData.global_ig) - (previousData.global_hy - previousData.global_ig) >= 0
                ? 'text-red-600'
                : 'text-green-600'
            }`}>
              {(latestData.global_hy - latestData.global_ig) - (previousData.global_hy - previousData.global_ig) >= 0 ? '+' : ''}
              {((latestData.global_hy - latestData.global_ig) - (previousData.global_hy - previousData.global_ig)).toFixed(0)} bps
            </p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow-md">
        <div className="mb-4">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">90-Day Spread Trends</h2>
        ...
        <ResponsiveContainer width="100%" height={400}>
          <LineChart data={chartData}>
            ...
            {showGlobalIG && <Line type="monotone" dataKey="global_ig" stroke="#3b82f6" name="Global IG" strokeWidth={2} dot={false} />}
                ...
                return (
                  <tr key={category} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {changes.current ? `${changes.current.toFixed(0)} bps` : 'N/A'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${
                      changes.daily > 0 ? 'text-red-600' : changes.daily < 0 ? 'text-green-600' : 'text-gray-500'
                    }`}>
                      {changes.daily !== 0 ? `${changes.daily > 0 ? '+' : ''}${changes.daily.toFixed(0)} bp` : '0 bp'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${
                      changes.weekly > 0 ? 'text-red-600' : changes.weekly < 0 ? 'text-green-600' : 'text-gray-500'
                    }`}>
                      {changes.weekly !== 0 ? `${changes.weekly > 0 ? '+' : ''}${changes.weekly.toFixed(0)} bp` : '0 bp'}
                    </td>
                    <td className={`px-6 py-4 whitespace-nowrap text-sm font-semibold ${
                      changes.monthly > 0 ? 'text-red-600' : changes.monthly < 0 ? 'text-green-600' : 'text-gray-500'
                    }`}>
                      {changes.monthly !== 0 ? `${changes.monthly > 0 ? '+' : ''}${changes.monthly.toFixed(0)} bp` : '0 bp'}
                    </td>
                  </tr>
                );
              })}
...
export default CorporateSpreadsPage;
