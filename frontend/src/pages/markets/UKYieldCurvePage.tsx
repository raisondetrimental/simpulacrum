...
import { getUKYieldCurve, refreshUKYieldCurve, UKYieldCurveData } from '../../services/marketsService';

const UKYieldCurvePage: React.FC = () => {
  const navigate = useNavigate();
  const [yieldData, setYieldData] = useState<UKYieldCurveData | null>(null);
  ...
  const loadYieldCurve = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getUKYieldCurve();
      setYieldData(data);
    } catch (err) {
      console.error('Error loading UK yield curve:', err);
  ...
  const handleRefresh = async () => {
    try {
      setRefreshing(true);
      setError(null);
      const data = await refreshUKYieldCurve();
      setYieldData(data);
    } catch (err) {
      console.error('Error refreshing UK yield curve:', err);
...
export default UKYieldCurvePage;
