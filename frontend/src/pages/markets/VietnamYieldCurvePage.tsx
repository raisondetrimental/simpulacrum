...
import { getVietnamYieldCurve, refreshVietnamYieldCurve, VietnamYieldCurveData } from '../../services/marketsService';

const VietnamYieldCurvePage: React.FC = () => {
  const navigate = useNavigate();
  const [yieldData, setYieldData] = useState<VietnamYieldCurveData | null>(null);
  ...
  const loadYieldCurve = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await getVietnamYieldCurve();
      setYieldData(data);
    } catch (err) {
      console.error('Error loading Vietnam yield curve:', err);
  ...
  const handleRefresh = async () => {
    try {
      setRefreshing(true);
      setError(null);
      const data = await refreshVietnamYieldCurve();
      setYieldData(data);
    } catch (err) {
      console.error('Error refreshing Vietnam yield curve:', err);
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <h3 className="text-sm font-medium text-gray-600 mb-1">Short-term (1Y)</h3>
                <p className="text-2xl font-bold text-blue-600">
                  {yieldData.yields.find(d => d.maturity === '1Y')?.yield?.last?.toFixed(2) || 'N/A'}%
                </p>
                <p className="text-xs text-gray-600 mt-1">
                  1M change: {yieldData.yields.find(d => d.maturity === '1Y')?.yield?.chg_1m?.toFixed(1) || 'N/A'} bp
                </p>
              </div>
            ...
            <div className="mt-4 bg-gray-50 rounded-lg p-4">
              ...
              <p className="text-sm text-gray-600">
                {(() => {
                  const oneYear = yieldData.yields.find(d => d.maturity === '1Y')?.yield?.last;
                  const tenYear = yieldData.yields.find(d => d.maturity === '10Y')?.yield?.last;

                  if (oneYear && tenYear) {
                    const spread = tenYear - oneYear;
                    if (spread > 1) {
...
export default VietnamYieldCurvePage;
