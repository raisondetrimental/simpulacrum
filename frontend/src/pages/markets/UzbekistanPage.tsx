...
import RelatedContentSidebar from '../../components/features/markets/RelatedContentSidebar';
...
const UzbekistanPage: React.FC = () => {
  ...
  const images = getCountryImages('uzbekistan');
  const currencyCode = getCurrencyCode('uzbekistan');
    ...
    const fetchData = async () => {
      try {
        ...
        const [fundamentalsResponse, completeDataResponse, fxRatesResponse] = await Promise.all([
          getCountryFundamentals('uzbekistan'),
          getCountryCompleteData('uzbekistan'),
        ...
        console.error('Error fetching Uzbekistan data:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="space-y-6">
                ...
                {countries.map((country) => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 hover:bg-blue-50 transition-colors ${
                      country.slug === 'uzbekistan' ? 'bg-blue-100 font-semibold' : ''
                    }`}
                  >
                    {country.name}
                  </button>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        {/* Related Content Sidebar (25%) */}
        <div className="lg:w-[25%]">
          <div className="lg:sticky lg:top-6">
            <RelatedContentSidebar
              country={{ name: 'Uzbekistan', slug: 'uzbekistan' }}
            />
          </div>
        </div>
      </div>
