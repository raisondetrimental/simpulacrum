/**
 * Agent Detail Page
 * View and edit agent details with contacts list
 */

import React, { useState, useEffect } from 'react';
import { useParams, Link, useNavigate } from 'react-router-dom';
import { Agent, AgentContact, AgentContactFormData, ApiResponse, MeetingHistoryEntry } from '../../types/agents';
...
const AgentDetail: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [agentRes, contactsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/agents/${id}`, { credentials: 'include' }),
      ...
      if (contactsResult.success) {
        const agentContacts = contactsResult.data!.filter(c => c.agent_id === id);
  ...
  const fetchDeals = async () => {
    if (!id) return;
    try {
      const response = await getAgentDeals(id);
  ...
  const handleSave = async (formData: Partial<Agent>) => {
    try {
      ...
      if (result.success && result.data) {
        ...
        setError(result.message || 'Failed to save agent');
      }
    } catch (err) {
  ...
  const handleToggleStar = async () => {
    ...
    try {
      const response = await toggleAgentStar(id);
        ...
        {!isEditing && (
          <div className="flex gap-2">
            <button
              ...
              title={agent.starred ? 'Remove star' : 'Add star'}
            >
              {agent.starred ? '★' : '☆'}
            </button>
            <button
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-gray-900">{agent.headquarters_location}</p>
              </div>
              ...
              <div>
                ...
                <p className="text-gray-900">{agent.agent_type}</p>
              </div>
            ...
            {agent.notes && (
              <div className="mt-4">
                ...
                <p className="text-gray-900 whitespace-pre-wrap">{agent.notes}</p>
              </div>
            )}
          </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {contacts.length === 0 ? (
          ...
          <div className="bg-gray-50 p-4 rounded-md text-sm text-gray-600">
            ...
            <p>Last Updated: {new Date(agent.last_updated).toLocaleString()}</p>
          </div>
        </>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md">
            ...
            <p className="text-red-600 font-semibold mb-4">
              Warning: This will also delete all {contacts.length} contact(s) in this agent!
            </p>
