...
const CorporatesTableView: React.FC = () => {
  ...
  const filteredCorporates = corporates.filter(corporate => {
    ...
    // Infrastructure type filters
    const matchesTransport = matchesFilter(corporate.infrastructure_types.transport_infra, transportFilter);
    const matchesEnergy = matchesFilter(corporate.infrastructure_types.energy_infra, energyFilter);

    // Region filters
    const matchesUsMarket = matchesFilter(corporate.regions.us_market, usMarketFilter);
    const matchesEm = matchesFilter(corporate.regions.emerging_markets, emFilter);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Corporates Table View</h1>
          <p className="text-gray-600 mt-1">{corporates.length} organizations</p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {sortedCorporates.map((corporate) => {
                  ...
                  if (corporate.infrastructure_types.energy_infra === 'Y') activeInfra.push('Energy');

                  // Get active regions
                  const activeRegions = [];
                  if (corporate.regions.us_market === 'Y') activeRegions.push('US');
                  if (corporate.regions.emerging_markets === 'Y') activeRegions.push('EM');
                  ...
                  return (
                    <React.Fragment key={corporate.id}>
                      ...
                      <tr className="bg-gray-50 table-row-stagger">
                          ...
                          {activeInfra.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {activeInfra.map(infra => (
                                <span key={infra} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">
                                  {infra}
                                </span>
                              ))}
                            </div>
                          ...
                          {activeRegions.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {activeRegions.map(region => (
                                <span key={region} className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">
                                  {region}
                                </span>
                              ))}
                            </div>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {corporate.contacts.length} contact(s)
                        </td>
                      </tr>
...
export default CorporatesTableView;
