/**
 * Liquidity Page - DES landing page with contact reminders and quick actions
 */

import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import ContactReminders from '../../components/features/capital-partners/ContactReminders';
import MeetingDetailsModal from '../../components/ui/MeetingDetailsModal';
import { Contact, getContacts, updateMeetingNote, deleteMeetingNote } from '../../services/capitalPartnersService';
  ...
  const recentMeetings = (() => {
    ...
    contacts.forEach(contact => {
      if (contact.meeting_history && contact.meeting_history.length > 0) {
        contact.meeting_history.forEach((meeting: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border-b border-gray-200 pb-4">
        <h1 className="text-3xl font-bold text-gray-900">Liquidity</h1>
        <p className="text-gray-600 mt-1">Manage capital partner relationships and investment strategies</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {/* Add Capital Partner */}
        <Link
          to="/liquidity/capital-partners?add=true"
        ...
        >
          <div className="flex items-center justify-center flex-col py-6">
            ...
            <h3 className="text-lg font-semibold text-gray-900">Add Capital Partner</h3>
            <p className="text-sm text-gray-600 mt-1">Create new capital partner</p>
          </div>
        </Link>

        {/* Meeting Notes - Quick Access */}
        <Link
          to="/liquidity/meeting"
        ...
        {/* Capital Partners - Main Interface */}
        <Link
        ...
        >
          <div className="flex items-center justify-center flex-col py-6">
            ...
            <p className="text-sm text-gray-600 mt-1">Browse partners & contacts</p>
          </div>
        </Link>
        ...
        >
          <div className="flex items-center justify-center flex-col py-6">
            ...
            <p className="text-sm text-gray-600 mt-1">Filter by investment preferences</p>
          </div>
        </Link>
      </div>
          ...
          <div className="space-y-3">
            {recentMeetings.map((item, index) => (
              <button
                ...
                className="w-full text-left border border-gray-200 rounded-md p-4 hover:bg-green-50 focus:outline-none focus:ring-2 focus:ring-green-500 transition-colors"
              >
                ...
                <p className="mt-2 text-xs font-semibold text-green-600">Open full details</p>
              </button>
            ))}
          </div>
        )}
